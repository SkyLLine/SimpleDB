...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		BTreeInternalPageIterator btiterator = new BTreeInternalPageIterator((BTreeInternalPage) getPage(tid, dirtypages, pid, perm));
...
//			System.out.print(t);
		...
		page.deleteTuple(t);
		newPage.insertTuple(t);
		BTreeInternalPage ParentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), t.getField(keyField));
		BTreeEntry btentry = new BTreeEntry(t.getField(keyField), page.getId(), newPage.getId());
		ParentPage.insertEntry(btentry);
        ...
        if(field.compare(Op.GREATER_THAN, t.getField(keyField))){
        	return newPage;
		}
		else
			{
...
//		newPage.insertEntry(entry);
		...
		BTreeEntry bTreeEntry = new BTreeEntry(entry.getKey(), page.pid, newPage.pid);
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		if(isRightSibling) {
			...
			entry.setRightChild(sibling.getId());
			...
			entry.setLeftChild(sibling.getId());
			parent.updateEntry(entry);
			page.setParentId(parent.getId());
			sibling.setParentId(parent.getId());
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		while(count > 0 && it.hasNext()){
			...
			parent.updateEntry(entry);
			parentEntry.setLeftChild(rightson);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
		int count = rightSibling.getNumEntries() - page.getNumEntries();
		count /= 2;
		Iterator<BTreeEntry> it = rightSibling.iterator();
		while(count > 0 && it.hasNext()){
			BTreeEntry entry = it.next();
			rightSibling.deleteKeyAndLeftChild(entry);
			BTreePageId leftson = entry.getLeftChild();
			updateParentPointer(tid, dirtypages, page.pid, leftson);
//			parent.deleteKeyAndRightChild(parentEntry);
			...
			parentEntry.setRightChild(leftson);
			page.insertEntry(parentEntry);
			parentEntry = entry;
			count--;
			System.out.print(2);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		if(rightPage.getRightSiblingId() != null){
			BTreeLeafPage rightsRight = (BTreeLeafPage)getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
			rightsRight.setLeftSiblingId(leftPage.getId());
		}
